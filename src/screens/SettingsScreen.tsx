...
export function SettingsScreen() {
  ...
  const testEmbedding = async () => {
    ...
    try {
      // Built-in model: prove inference works on this machine, no HTTP involved.
      if (settings.embedding.kind === "local") {
        const [vector] = await ipc.localEmbedTexts(["lattice"]);
        setEmbTest({
          status: "ok",
          detail: `On-device model produced ${vector?.length ?? 0} dims`,
        });
        return;
      }
              ...
              onChange={(e) =>
                setSettings({
                  ...
                  embedding: {
                    ...settings.embedding,
                    ...e,
                    // The built-in model's dimensions aren't configurable.
                    ...(e.kind === "local" ? { dimensions: LOCAL_EMBEDDING.dimensions } : {}),
                  },
                })
              }
              apiKey={embeddingKey}
